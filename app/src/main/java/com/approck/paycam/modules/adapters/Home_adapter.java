...
public class Home_adapter extends RecyclerView.Adapter {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        view = inflater.inflate(R.layout.catigory_item, viewGroup, false);
...
////        Log.d("BookServiceError"," item : "+item.toString());
//
        holder.name.setText(item.getTitel());
        Glide.with(context).load(item.getIcon()).into(holder.photo);
        int colorIndex = position % allColors.length;
        holder.item_lin.setBackgroundColor(Color.parseColor(allColors[colorIndex]));
